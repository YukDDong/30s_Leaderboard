...
- 보기 전용 페이지: `/league`
- 관리자 페이지: `/admin`, `/admin/league`, `/admin/league-tournament`
...
- legacy 클라이언트인 `src/legacy/supabase-client.js`가 `https://esm.sh/@supabase/supabase-js`를 직접 import 합니다.
...
`/admin`, `/admin/league`, `/admin/league-tournament`
...
6. `/admin` 열기
7. 관리자 비밀번호 입력
8. `/admin/league`에서 팀 2개 이상 추가
9. 경기 생성
10. 점수 입력
11. `/league` 보기 페이지에서 동일 데이터가 읽히는지 확인
...
- `/`에서 허브 버튼 3개가 보임
- `/league`에서 경기 테이블과 순위표가 보임
- `/league`에서 수정 버튼이 없음
- `/admin`, `/admin/league`, `/admin/league-tournament`에서 비밀번호 입력 전에는 수정 UI가 잠김
- 인증 성공 후 팀 추가 가능
- 9팀이면 경기 생성 후 총 36경기
- `6 : 2` 입력 시 승점과 득실차 반영
- 경기 결과 초기화 가능
...
- 관리자 작업은 `/admin` 아래에서만 하고, 일반 공유는 `/league`와 `/league-tournament`를 사용합니다.
